...
"""
    ...
    * Sn0:    Time before difficulty adjustment 
    * Tho0:   Theoretical average time for a block to be mined => 10 min (Bitcoin protocol)
    * n0:     Number of blocks to be mined before difficulty adjustment
    * B:      Correction factor 'miniLambda' : B = Sn0 / (n0*Tho0)
    * Difficulty adjustment occurs when 2016 blocks have been mined => normally 2 weeks
    * t is the 'break time' or time before profitability for Selfish miners
        -> as Selfish miners now decided to invest more resources than honest miners
        -> so that difficulty decreases and they could mine even quicker afterwards
    We seek to find t.
    
    PnL = R - C  # What is this?
    compare PnL for honest vs selfish miners
    
    __nb.simulations become number of blocks to be mined
...
class SelfishMining:
    ...
    def write_file(self):
        stats_result = [self.__alpha, self.__gamma, self.__nb_simulations, self.__currentTimestamp, \
                        self.__totalValidatedBlocks, self.__honestValidBlocks, self.__selfishValidBlocks, \
    ...
    def actualize_results(self, ChangeDifficulty=False):
        ...
        # Revenue
        if self.__honestValidBlocks or self.__selfishValidBlocks:
