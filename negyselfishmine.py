...
class Block:
    ...
    def __str__(self):
        return "[" + "height = " + str(self.height) + ", time = " + str(self.timestamp) + ", " + str(
            self.difficulty) + ", total_work = " + str(self.total_work) + ", author = " + str(self.author) + "]"
...
class Coin:
        ...
        while self.latest_block.height < 0:
            ...
            if random.random() <= INIT_HASH_POWER / self.latest_block.difficulty:
                self.latest_block = Block(
                    getattr(self, 'calc_' + self.__class__.__name__ + '_difficulty')(temp_time, self.latest_block),
                    temp_time,
    ...
    def queue_block_for_broadcast(self, block):
        ...
        if timestep_run == True:
            ...
            profit_data[data_index][block.number - 1].append(
                winnings[selfish_ID] / ((global_time - self.start_time) / 60.0))
    ...
    def __init__(self):
        ...
        for k in range(2017):
            self.latest_block = Block(INIT_HASH_POWER * self.nPowTargetSpacing,
                                      (2016 - k) * -self.nPowTargetSpacing,
                                      # Negative timestamps starts global time at 0
...
class ETHCoin(Coin):
    ...
    def create_block(self, time, miner):
        ...
        assert time > parent.timestamp, "errZeroBlockTime: time less than parent timestep %s %s." % (
        str(time), str(parent))
    ...
    def queue_block_for_broadcast(self, block):
        ...
        if timestep_run == True:
            ...
            # global time (seconds), profit (minutes)
            profit_data[data_index][block.number - 1].append(
                blocks_won[selfish_ID] / ((global_time - self.start_time) / 60.0))
...
class Miner:
    ...
    def get_difficulty(self):
        if self.coin.__class__.__name__ == "ETHCoin":  # ETH difficulty must always be recalculated with new time
            return getattr(self.coin, 'calc_' + self.coin.__class__.__name__ + '_difficulty')(global_time,
                                                                                              self.latest_block)
...
class HonestMiner(Miner):
    ...
    def get_gamma_difficulty(self):
        assert self.fork_choice[
                   0].height == self.latest_block.height, "heights are different\n%s\n%s\nlatest sm block %s" % (
        self.fork_choice[0], self.latest_block, self.coin.miners[0].latest_block)
        assert self.fork_choice[0].total_work == self.latest_block.total_work, "work is different\n%s\n%s" % (
        self.fork_choice[0], self.latest_block)
        return getattr(self.coin, 'calc_' + self.coin.__class__.__name__ + '_difficulty')(global_time,
                                                                                          self.fork_choice[0])
...
def run_test(coin_type, miner_type, test_type, add_hash_power, alpha, gamma):
    ...
    for i in range(len(new_hash_rate)):
        ...
        print
        "hash_rate = " + str(selfish_hash_rate)
    ...
    if timestep_run == True:
        ...
        with open(data_path + coin_type + "_" + miner_type + "_timestep_profit_blocks_per_minute" + add + ".data",
                  "w") as k:
            k.write(
                "block_number 0.1_avg 0.1_stddev 0.33_avg 0.33_stddev 0.49_avg 0.49_stddev 0.1_expected 0.33_expected 0.49_expected\n")
            for i in range(blocks_produced):
                k.write(str(i) + " ")
                for j in range(len(new_hash_rate)):
                    try:
        ...
        with open(data_path + coin_type + "_" + miner_type + "_blocks_per_minute_" + str(gamma) + add + ".data",
                  "w") as file:
        ...
        with open(data_path + coin_type + "_" + miner_type + "_proportion_earned_" + str(gamma) + add + ".data",
                  "w") as file:
            file.write("hash_rate " + str(gamma) + "_avg " + str(gamma) + "_stddev expected_honest\n")
        ...
        with open(data_path + coin_type + "_" + miner_type + "_system_blocks_per_minute_" + str(gamma) + add + ".data",
                  "w") as file:
            file.write("hash_rate " + str(gamma) + "_avg " + str(gamma) + "_stddev expected_honest\n")
...
def exception_usage(message):
    ...
    "Error:" + message
    print
    "usage: python selfishminesim.py -coin <coin name> -test <test name> -miner_type <mine type> -num_blocks <integer> -num_runs <integer>"
    print
    "-coin: BTC, BSV, XMR, or ETH"
    print
    "-test: timestep or whole"
    print
    "-add_hash_power: true or false"
    print
    "-miner_type: honest or selfish"
    print
    "-num_blocks: default is 1000"
    print
    "-num_runs: default is 100"
    print
    "-alpha: one float between 0 and 0.5 to simulate. (optional)"
    print
    "-gamma: 0, 0.5, or 1. Default is 0"
    exit()
